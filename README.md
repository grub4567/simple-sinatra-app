...
# Overview
The application will be deployed into AWS. This project contains a Packer template to create a
Amazon Machine Image (AMI) containing the application served using Puma and NGINX. Terraform
scripts will provision infrastructure including servers based on this AMI. The AWS infrastructure
includes an Autoscaling group to provide robustness and rolling deployments of new AMIs.
...
NGINX reverse proxies HTTP/80 to. A systemd unit for Puma is provided to ensure that it restarts
...
- The VPC is furnished with an internet gateway and a route table connecting the subnets to the
  internet gateway.
- Security groups:
  ...
  - One for a loadbalancer. It allows HTTP from anywhere and HTTP to the application server
    security group.
...
- An autoscaling group using the launch template and the loadbalancer target group. The autoscaling
  group is provisioned by Terraform but via Cloudformation to provide rolling deployments. See
...
## Prerequisites
- A POSIX shell environment, including the programs:
  - [Terraform](https://www.terraform.io/downloads.html) version 0.12 or higher.
  - [Packer](https://www.packer.io/downloads.html)
  - [AWS CLI](https://aws.amazon.com/cli/)
  - make
  - curl
- Credentials to an AWS account which the application will be deployed into.
  - The credential's user should have administrator access (a minimal IAM policy is a TODO, see
    shortcomings)
...
## Quick
- Make sure that the AWS CLI is installed and your default AWS credentials in your environment
  are correct. You can follow the guide [here](https://aws.amazon.com/cli/) to configure your
  environment.
- Ensure that the required programs are installed (Terraform, Packer, make, curl).
- Clone the repo:
```sh
...
```
- You should now have an AMI containing the application in your AWS account.
- Review the Terraform variables. In particular, review the variable which is the path to the
  public SSH key which will be added to the provisioned servers and make sure it is correct.
...
```
- The application server should come up and be accessible on the loadbalancer domain name output
  by the Terraform command. This might take a minute or so:
```sh
...
## Design Choices
I wanted to demonstrate that I could build a simple and typical AWS deployment using commonplace
...
- The application instances have public IPs, which is not ideal. Adding bastion instances
  ...
  have public IPs.
- An IAM policy containing the minimal permissions for being able to run 
